...
import React from 'react';
import { Chapter } from './components/Data/course';  // Ensure you have the right import
import { ChevronDown, Play } from 'lucide-react';
        ...
        <div className="space-y-4">
            {chapters.map((chapter, index) => (
                <div key={chapter.id} className="border rounded-lg overflow-hidden">
                    <div
                        className="flex justify-between items-center p-4 bg-gray-50 cursor-pointer hover:bg-gray-100"
                        onClick={() => onChapterClick(index)}  // Toggle onClick
                    >
                        <h3 className="text-lg font-medium">
                            {chapter.id}. {chapter.title}
                        </h3>
                        <ChevronDown
                            className={`w-5 h-5 transform transition-transform ${activeChapter === index ? 'rotate-180' : ''}`}
                        />
                    </div>

                    {/* Show chapter videos if it's the active chapter */}
                    {activeChapter === index && (
                        <div className="p-4 space-y-2">
                            {chapter.videos.map((video) => (
                                <div
